...
////////////////////
app.get('/', function (req, res) {
    res.send('<ul>' + '<li>Download <a href="/files/amazing.txt">amazing.txt</a>.</li>' + '<li>Download <a href="/files/missing.txt">missing.txt</a>.</li>' + '</ul>');
});
...
app.post('/', function (req, res) {
    ...
    res.send(mformat('\nuploaded %s (%d Kb) to %s as %s', req.files.image.name, req.files.image.size / 1024 | 0, req.files.image.path, req.body.title));
});
...
app.get('/users/:from-:to', function (req, res) {
    var from = req.params.from, to = req.params.to, names = users.map(function (user) {
        return user.name;
    });
...
app.resource = function (path, obj) {
    ...
    this.get(path + '/:a..:b.:format?', function (req, res) {
        var a = parseInt(req.params.a, 10), b = parseInt(req.params.b, 10), format = req.params.format;
...
app.use('/api', function (req, res, next) {
    ...
    // key isnt present
    if (!key)
        return next(merror(400, 'api key required'));

    // key is invalid
    if (!~apiKeys.indexOf(key))
        return next(merror(401, 'invalid api key'));
...
app.get('/api/user/:name/repos', function (req, res, next) {
    var name = req.params.name, user = userRepos[name];
...
function test_general() {
    ...
    app.render('email', { name: 'Tobi' }, function () {
    });
}
...
function test_response() {
    ...
    res.render('user', { name: 'Tobi' }, function () {
    });
}
